...
{
    public static NamelessItems Parse(string text, string starttoken)
    {
        using var p = new SqlParser(text);
        return Parse(p, starttoken);
    }

    public static NamelessItems Parse(SqlParser parser, string starttoken)
    {
        ...
        var oc = new NamelessItems(starttoken);
